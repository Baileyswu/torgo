...
local=mdata/$wav_type/local_$subset
tmp=$(mktemp -d /tmp/${USER}_XXXXX)
lexicon=mdata/$wav_type/local/dict/lexicon.txt
...
test=split/$wav_type/$spk/lang_test_$subset
mkdir -p $test
for f in phones.txt words.txt phones.txt L.fst L_disambig.fst phones; do
    cp -r split/$wav_type/$spk/lang/$f $test
done
cat $local/lm.arpa | \
